...
startValidator () {
    ...
    if [[ -z "$MODE" || "$MODE" == native-to-erc ]]; then
      docker-compose $1 run $2 $db_env -d oracle yarn watcher:signature-request
      docker-compose $1 run $2 $db_env -d oracle yarn watcher:collected-signatures
      docker-compose $1 run $2 $db_env -d oracle yarn watcher:affirmation-request
    fi
    if [[ -z "$MODE" || "$MODE" == erc-to-erc ]]; then
      docker-compose $1 run $2 $db_env -d oracle-erc20 yarn watcher:signature-request
      docker-compose $1 run $2 $db_env -d oracle-erc20 yarn watcher:collected-signatures
      docker-compose $1 run $2 $db_env -d oracle-erc20 yarn watcher:affirmation-request
      docker-compose $1 run $2 $db_env -d oracle-erc20 yarn watcher:transfer
    fi
    if [[ -z "$MODE" || "$MODE" == erc-to-native ]]; then
      ...
      docker-compose $1 run $2 $db_env -d oracle-erc20-native yarn worker:convert-to-chai
    fi
    ...
    docker-compose $1 run $2 $db_env -d oracle-erc20-native yarn sender:home
    docker-compose $1 run $2 $db_env -d oracle-erc20-native yarn sender:foreign
}
